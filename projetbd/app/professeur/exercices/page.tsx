...
type Exercise = {
  id?: number // Changé de string à number pour correspondre au type dans la base de données
  title: string
  subtitle: string
  category: string
...
export default function ExercisesPage() {
  ...
  const fetchExercises = async () => {
    ...
    try {
      ...
      console.log("Données brutes reçues:", data) // Log pour déboguer
      ...
      const formattedExercises = data.map((exercise: any) => {
        ...
        return {
          ...
          status: exercise.status || "Publié",
          statusClass: getStatusClass(exercise.status || "Publié"),
          fileUrl: fileUrl,
  ...
  const handleViewExercise = (exercise: Exercise) => {
    console.log("Visualisation de l'exercice:", exercise) // Log pour déboguer
    setViewExercise(exercise)
  }

  const handleEditExercise = (exercise: Exercise) => {
    console.log("Modification de l'exercice:", exercise) // Log pour déboguer
    if (!exercise.id) {
      showNotification("error", "Impossible de modifier cet exercice: ID manquant")
      return
    }
  ...
  const handleDeleteExercise = (exercise: Exercise) => {
    console.log("Suppression de l'exercice:", exercise) // Log pour déboguer
    if (!exercise.id) {
      showNotification("error", "Impossible de supprimer cet exercice: ID manquant")
      return
    }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log(`Envoi de la requête PUT à /sujets/${editExercise.id}`) // Log pour déboguer
  ...
  const confirmDeleteExercise = async () => {
    if (!deleteExercise || !deleteExercise.id) {
      showNotification("error", "Impossible de supprimer cet exercice: ID manquant")
      return
    }
